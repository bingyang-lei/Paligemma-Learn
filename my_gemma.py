...
import math
from my_siglip import SiglipVisionConfig, SiglipVisionModel
...
class GemmaConfig():
    ...
    def __init__(
        self,
        vocab_size,
        hidden_size, # embeding size
        intermediate_size,
        num_hidden_layers,
        num_attention_heads, # query的头数
        num_key_value_heads, # key和value的头数
        head_dim=256,
        max_position_embeddings=8192, # 和RoPE有关
        rms_norm_eps=1e-6,
        rope_theta=10000.0,
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def tie_weights(self): 
        # 用于将attention论文中最前面和最后面的两个线性层进行权重共享，
        # 因为这两个线性层实际上的工作就是彼此的逆过程
